...
function compassBin() {
  return process.env.COMPASS_BIN || "compass";
}
...
var GOVERNED_ACTIONS = {
  ...
  // Onboarding actions — deploy the per-owner Compass smart-account
  // contracts (Earn / Credit / Tokenized) that the actual usage scenarios
  // require. Each create-account call returns an unsigned EVM tx that
  // deploys a proxy on Base; Sly's executor signs + broadcasts via CDP.
  //
  // Gated on the synthetic venue 'compass-onboarding' so tenants can
  // allow / deny the whole class of setup actions with one policy
  // toggle rather than per-surface entries.
  governed_earn_create_account: {
    subcommand: "earn:create_account",
    executable: true,
    toIntentParams: (a) => ({
      chain: a.chain ?? "base",
      amount: "0",
      venue_type: "compass-onboarding"
    }),
    toCliArgs: (a) => [
      "earn",
      "create-account",
  ...
  governed_credit_create_account: {
    subcommand: "credit:create_account",
    executable: true,
    toIntentParams: (a) => ({
      chain: a.chain ?? "base",
      amount: "0",
  ...
  governed_tokenized_create_account: {
    subcommand: "tokenized:create_account",
    executable: true,
    toIntentParams: (a) => ({
      chain: a.chain ?? "base",
      amount: "0",
  ...
  governed_credit_borrow: {
    ...
    // With --gas-sponsorship=false (our default) Compass returns a plain
    // Safe execTransaction we can sign+broadcast via CDP. The server-
    // factory check on compass.data.transaction safely falls through to
    // the payload-return path when Compass returns EIP-712 typed data
    // instead (e.g., if gas-sponsorship gets enabled later).
    executable: true,
    toIntentParams: (a) => ({ chain: a.chain ?? "base", amount: String(a.amount), venue_type: a.venue_type ?? `aave-credit:${a.borrow_token}` }),
    // Supports both plain borrow-against-existing-collateral and the
    // atomic supply+borrow used by the "Seed Aave collateral" scenario.
    // When `token_in`, `collateral_token`, and `amount_in` are all
    // present, Compass pulls `amount_in` of `token_in` from the Safe,
    // swaps it (if token_in != collateral_token) and supplies it as
    // collateral, then executes the borrow — all in one Safe tx.
    toCliArgs: (a) => {
      const base = [
        "credit",
        "borrow",
      ...
      if (a.token_in && a.collateral_token && a.amount_in) {
        return [
          ...base,
          "--token-in",
          a.token_in,
  ...
  governed_credit_repay: {
    ...
    // Same Safe execTransaction shape as borrow when gas-sponsorship is off.
    executable: true,
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (spec) {
        ...
        const createAccountData = compass.data;
        const surfaceFromSubcommand = spec.subcommand === "credit:create_account" ? "credit" : spec.subcommand === "earn:create_account" ? "earn" : spec.subcommand === "tokenized:create_account" ? "tokenized" : null;
        const safeAddress = createAccountData ? createAccountData.credit_account_address ?? createAccountData.earn_account_address ?? createAccountData.tokenized_account_address : void 0;
        const validSafe = safeAddress && /^0x[0-9a-fA-F]{40}$/.test(safeAddress) ? safeAddress : void 0;
        if (spec.executable && decision.evaluation_id && compass.data?.transaction) {
          const exec = await executeIntent(ctx, {
            ...
            ...validSafe ? { safe_address: validSafe } : {}
          });
          return textResult({ governed: true, decision: "approve", evaluation_id: decision.evaluation_id, ...exec });
        }
        if (validSafe && surfaceFromSubcommand && !compass.data?.transaction) {
          try {
            await fetch(`${ctx.apiUrl}/v1/onboarding/compass-demo/register-safe`, {
              method: "POST",
              headers: { Authorization: `Bearer ${ctx.apiKey}`, "Content-Type": "application/json" },
              body: JSON.stringify({ safe_address: validSafe, agent_id: args.agent_id, surface: surfaceFromSubcommand })
            });
          } catch {
          }
          return textResult({
            ...
            decision: "approve",
            evaluation_id: decision.evaluation_id,
            already_deployed_at: validSafe,
            note: `${surfaceFromSubcommand} Safe already deployed at ${validSafe}. Re-registered with Sly's onboarding endpoint.`
          });
        }
        return textResult({
          governed: true,
