#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for sigil-skills.
#
# Reads SIGIL_API_KEY from .env.local, provisions a Sigil Demo account +
# Sigil Renter Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning sigil-skills against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Sigil Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Sigil Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Sigil Demo",
    "metadata": { "onboarded_via": "sigil-skills/scripts/onboard.sh" }
  }')
...
# ── 2. Sigil Renter Agent ──────────────────────────────────────────────
EXISTING=$(get_json "/v1/agents?search=Coral%20Buyer%20Agent&limit=5" | python3 -c "
import json, sys
d = json.load(sys.stdin)
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Sigil Renter Agent':
        print(a.get('id', '')); break
")

AGENT_ID="$EXISTING"
AGENT_TOKEN="${SIGIL_RENTER_AGENT_TOKEN:-}"

if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Sigil Renter Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Sigil Renter Agent\",
    \"description\": \"Skill renter — rents time-bounded skill grants from other agents with auto-revoke\",
...
d=json.load(sys.stdin); data=d.get('data',d); creds=data.get('credentials', d.get('credentials',{})); print(creds.get('token',''))")
  ...
  echo "  ⚠ Agent $AGENT_ID exists but SIGIL_RENTER_AGENT_TOKEN is missing — Sly only returns the token at creation."
...
lines = [
    "",
    "# === Auto-written by sigil-skills/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
    f"SIGIL_RENTER_ACCOUNT_ID=$ACCOUNT_ID",
    f"SIGIL_RENTER_AGENT_ID=$AGENT_ID",
]
if "$AGENT_TOKEN":
    lines.append(f"SIGIL_RENTER_AGENT_TOKEN=$AGENT_TOKEN")
lines.append("# === End auto-written block ===")
block = "\n".join(lines) + "\n"
current = env_path.read_text() if env_path.exists() else ""
current = re.sub(
    r"\n?# === Auto-written by sigil-skills/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3263"
