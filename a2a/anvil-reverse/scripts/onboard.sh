#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for anvil-reverse.
#
# Reads ANVIL_API_KEY from .env.local, provisions a Anvil Demo account +
# Anvil Buyer Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning anvil-reverse against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Anvil Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Anvil Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Anvil Demo",
    "metadata": { "onboarded_via": "anvil-reverse/scripts/onboard.sh" }
  }')
...
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Anvil Buyer Agent':
        print(a.get('id', '')); break
")

AGENT_ID="$EXISTING"
AGENT_TOKEN="${ANVIL_BUYER_AGENT_TOKEN:-}"

if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Anvil Buyer Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Anvil Buyer Agent\",
    \"description\": \"Reverse-auction buyer — posts intents, evaluates seller bids, governed by Sly\",
...
d=json.load(sys.stdin); data=d.get('data',d); creds=data.get('credentials', d.get('credentials',{})); print(creds.get('token',''))")
  ...
  echo "  ⚠ Agent $AGENT_ID exists but ANVIL_BUYER_AGENT_TOKEN is missing — Sly only returns the token at creation."
...
lines = [
    "",
    "# === Auto-written by anvil-reverse/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
    f"ANVIL_BUYER_ACCOUNT_ID=$ACCOUNT_ID",
    f"ANVIL_BUYER_AGENT_ID=$AGENT_ID",
]
if "$AGENT_TOKEN":
    lines.append(f"ANVIL_BUYER_AGENT_TOKEN=$AGENT_TOKEN")
lines.append("# === End auto-written block ===")
block = "\n".join(lines) + "\n"
current = env_path.read_text() if env_path.exists() else ""
current = re.sub(
    r"\n?# === Auto-written by anvil-reverse/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3254"
