#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for span-chatgpt-mock.
#
# Reads SPAN_API_KEY from .env.local, provisions a Span Demo account +
# Span Buyer Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning span-chatgpt-mock against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Span Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Span Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Span Demo",
    "metadata": { "onboarded_via": "span-chatgpt-mock/scripts/onboard.sh" }
  }')
...
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Span Buyer Agent':
        print(a.get('id', '')); break
")

AGENT_ID="$EXISTING"
AGENT_TOKEN="${SPAN_AGENT_TOKEN:-}"

if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Span Buyer Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Span Buyer Agent\",
    \"description\": \"Span shopping agent — Claude side of the cross-platform broker viewer\",
...
d=json.load(sys.stdin); data=d.get('data',d); creds=data.get('credentials', d.get('credentials',{})); print(creds.get('token',''))")
  ...
  echo "  ⚠ Agent $AGENT_ID exists but SPAN_AGENT_TOKEN is missing — Sly only returns the token at creation."
...
lines = [
    "",
    "# === Auto-written by span-chatgpt-mock/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
    f"SPAN_ACCOUNT_ID=$ACCOUNT_ID",
    f"SPAN_AGENT_ID=$AGENT_ID",
]
if "$AGENT_TOKEN":
    lines.append(f"SPAN_AGENT_TOKEN=$AGENT_TOKEN")
lines.append("# === End auto-written block ===")
block = "\n".join(lines) + "\n"
current = env_path.read_text() if env_path.exists() else ""
current = re.sub(
    r"\n?# === Auto-written by span-chatgpt-mock/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3221"
