#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for lume-goods.
#
# Reads ASTER_API_KEY from .env.local, provisions a Aster Demo account +
# Aster Buyer Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning lume-goods against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Aster Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Aster Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Aster Demo",
    "metadata": { "onboarded_via": "lume-goods/scripts/onboard.sh" }
  }')
...
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Aster Buyer Agent':
        print(a.get('id', '')); break
")
...
if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Aster Buyer Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Aster Buyer Agent\",
    \"description\": \"Buyer agent that initiates ACP checkouts against the Lume Goods storefront\",
...
lines = [
    "",
    "# === Auto-written by lume-goods/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
...
current = re.sub(
    r"\n?# === Auto-written by lume-goods/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3231"
