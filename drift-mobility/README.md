...
Mobility micropay wallet — parking, tolls, charging via x402.
...
echo "DRIFT_API_KEY=pk_test_…" > .env.local

# 2. Provision agents + accounts on your tenant. Idempotent.
pnpm onboard
# → writes `SLY_API_URL` + `DRIFT_ACCOUNT_ID` + `DRIFT_AGENT_ID` + `DRIFT_AGENT_TOKEN` to .env.local
...
- Creates a **Drift Demo** business account (KYB tier 2)
- Creates a **Drift Mobility Agent** under it
