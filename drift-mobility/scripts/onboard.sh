#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for drift-mobility.
#
# Reads DRIFT_API_KEY from .env.local, provisions a Drift Demo account +
# Drift Mobility Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning drift-mobility against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Drift Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Drift Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Drift Demo",
    "metadata": { "onboarded_via": "drift-mobility/scripts/onboard.sh" }
  }')
...
# ── 2. Drift Mobility Agent ──────────────────────────────────────────────
EXISTING=$(get_json "/v1/agents?search=Coral%20Buyer%20Agent&limit=5" | python3 -c "
import json, sys
d = json.load(sys.stdin)
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Drift Mobility Agent':
        print(a.get('id', '')); break
")

AGENT_ID="$EXISTING"
AGENT_TOKEN="${DRIFT_AGENT_TOKEN:-}"

if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Drift Mobility Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Drift Mobility Agent\",
    \"description\": \"Mobility micropay agent — parking/tolls/charging via x402\",
...
d=json.load(sys.stdin); data=d.get('data',d); creds=data.get('credentials', d.get('credentials',{})); print(creds.get('token',''))")
  ...
  echo "  ⚠ Agent $AGENT_ID exists but DRIFT_AGENT_TOKEN is missing — Sly only returns the token at creation."
...
lines = [
    "",
    "# === Auto-written by drift-mobility/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
    f"DRIFT_ACCOUNT_ID=$ACCOUNT_ID",
    f"DRIFT_AGENT_ID=$AGENT_ID",
]
if "$AGENT_TOKEN":
    lines.append(f"DRIFT_AGENT_TOKEN=$AGENT_TOKEN")
lines.append("# === End auto-written block ===")
block = "\n".join(lines) + "\n"
current = env_path.read_text() if env_path.exists() else ""
current = re.sub(
    r"\n?# === Auto-written by drift-mobility/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3251"
