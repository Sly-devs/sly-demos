#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for mint-business.
#
# Reads MINT_API_KEY from .env.local, provisions a Mint Demo account +
# Mint Business Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning mint-business against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Mint Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Mint Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Mint Demo",
    "metadata": { "onboarded_via": "mint-business/scripts/onboard.sh" }
  }')
...
if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Mint Business Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Mint Business Agent\",
    \"description\": \"Runs an agent-owned micro-business — manages inventory, P&L, dividends\",
...
d=json.load(sys.stdin); data=d.get('data',d); creds=data.get('credentials', d.get('credentials',{})); print(creds.get('token',''))")
  ...
  echo "  ⚠ Agent $AGENT_ID exists but MINT_AGENT_TOKEN is missing — Sly only returns the token at creation."
...
lines = [
    "",
    "# === Auto-written by mint-business/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
    f"MINT_ACCOUNT_ID=$ACCOUNT_ID",
    f"MINT_AGENT_ID=$AGENT_ID",
]
if "$AGENT_TOKEN":
    lines.append(f"MINT_AGENT_TOKEN=$AGENT_TOKEN")
lines.append("# === End auto-written block ===")
block = "\n".join(lines) + "\n"
current = env_path.read_text() if env_path.exists() else ""
current = re.sub(
    r"\n?# === Auto-written by mint-business/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3265"
