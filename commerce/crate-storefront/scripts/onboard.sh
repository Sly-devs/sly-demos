#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for crate-storefront.
#
# Reads CORAL_API_KEY from .env.local, provisions a Coral Demo account +
# Coral Buyer Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning crate-storefront against $API_URL …"
echo
...
if [[ -z "$ACCOUNT_ID" ]]; then
  ...
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Coral Demo",
    "metadata": { "onboarded_via": "crate-storefront/scripts/onboard.sh" }
  }')
...
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Coral Buyer Agent':
        print(a.get('id', '')); break
")
...
if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Coral Buyer Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Coral Buyer Agent\",
    \"description\": \"Buyer agent that initiates ACP checkouts against the Crate storefront\",
...
lines = [
    "",
    "# === Auto-written by crate-storefront/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
...
current = re.sub(
    r"\n?# === Auto-written by crate-storefront/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
