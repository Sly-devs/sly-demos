#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for echo-attention.
#
# Reads ECHO_API_KEY from .env.local, provisions a Echo Demo account +
# Echo Attention Agent on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning echo-attention against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Echo Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Echo Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Echo Demo",
    "metadata": { "onboarded_via": "echo-attention/scripts/onboard.sh" }
  }')
...
# ── 2. Echo Attention Agent ──────────────────────────────────────────────
EXISTING=$(get_json "/v1/agents?search=Coral%20Buyer%20Agent&limit=5" | python3 -c "
import json, sys
d = json.load(sys.stdin)
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Echo Attention Agent':
        print(a.get('id', '')); break
")

AGENT_ID="$EXISTING"
AGENT_TOKEN="${ECHO_AGENT_TOKEN:-}"

if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Echo Attention Agent…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Echo Attention Agent\",
    \"description\": \"Sell-my-attention agent — brand offers in x402 micropay out\",
...
d=json.load(sys.stdin); data=d.get('data',d); creds=data.get('credentials', d.get('credentials',{})); print(creds.get('token',''))")
  ...
  echo "  ⚠ Agent $AGENT_ID exists but ECHO_AGENT_TOKEN is missing — Sly only returns the token at creation."
...
lines = [
    "",
    "# === Auto-written by echo-attention/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
    f"ECHO_ACCOUNT_ID=$ACCOUNT_ID",
    f"ECHO_AGENT_ID=$AGENT_ID",
]
if "$AGENT_TOKEN":
    lines.append(f"ECHO_AGENT_TOKEN=$AGENT_TOKEN")
lines.append("# === End auto-written block ===")
block = "\n".join(lines) + "\n"
current = env_path.read_text() if env_path.exists() else ""
current = re.sub(
    r"\n?# === Auto-written by echo-attention/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3253"
