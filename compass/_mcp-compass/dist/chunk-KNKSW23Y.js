...
var tools = [
  ...
  {
    name: "governed_perps_deposit",
    description: "Post USDC as margin into the Hyperliquid perps account (funded via an Arbitrum USDC permit), GATED by Sly. STEP-UP: requires an active 'compass:perps' scope grant \u2014 without one the gate denies `scope_required:compass:perps`. The deposit amount is the at-risk margin and is balance-checked against the agent's USDC. Deny \u2192 reasons.",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string" },
        owner: { type: "string" },
        amount: { type: "string", description: "USDC amount to post as margin." }
      },
      required: ["agent_id", "owner", "amount"]
    }
  },
  {
    name: "governed_perps_withdraw",
    description: "Withdraw posted margin from the Hyperliquid perps account back to the owner wallet (Arbitrum USDC), GATED by Sly. De-risks the position; no step-up scope required, only the standing policy (venue allowlist + caps).",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string" },
        owner: { type: "string" },
        amount: { type: "string", description: "USDC amount to withdraw from margin." }
      },
      required: ["agent_id", "owner", "amount"]
    }
  },
