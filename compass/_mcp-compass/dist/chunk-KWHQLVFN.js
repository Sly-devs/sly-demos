...
// src/sign-typed-data-client.ts
async function signTypedDataViaSly(config, opts) {
  const res = await fetch(`${config.apiUrl}/v1/policy/sign-typed-data`, {
    method: "POST",
    headers: { Authorization: `Bearer ${config.apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify(opts)
  });
  ...
  const body = raw && typeof raw === "object" && "data" in raw && raw.data ? raw.data : raw;
  if (!res.ok) {
    return { signed: false, error: body.error || `sign-typed-data HTTP ${res.status}` };
  }
  return body;
}
...
// src/server-factory.ts
var BROADCASTABLE_CHAINS = /* @__PURE__ */ new Set(["base", "ethereum", "arbitrum"]);
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (spec) {
        ...
        if (evalId && data?.transaction && BROADCASTABLE_CHAINS.has(p.chain)) {
          const exec = await executeIntent(ctx, {
            agent_id: args.agent_id,
            evaluation_id: evalId,
            chain: p.chain,
            unsigned_transaction: data.transaction
          });
          return textResult({ governed: true, decision: "approve", evaluation_id: evalId, ...exec });
        }
        if (evalId && data?.typed_data) {
          const signed = await signTypedDataViaSly(ctx, {
            agent_id: args.agent_id,
            evaluation_id: evalId,
          ...
          if (signed.error) {
            return textResult({
              ...
              sign_error: signed.error
            });
          }
          return textResult({
            governed: true,
            executed: false,
            // signed, not yet submitted on-chain
            signed: true,
            decision: "approve",
            evaluation_id: evalId,
            ...
            owner: signed.owner,
            compass_payload: compass.data,
            note: "Sly signed the EIP-712 typed data with the agent CDP wallet \u2014 the matching one-shot compass grant is consumed. Hand the signature to `compass " + spec.subcommand.split(":")[0] + " execute --signature <sig>` to submit to " + (spec.subcommand.startsWith("perps") ? "Hyperliquid" : "the target protocol") + ". Bilateral receipt = evaluation_id + (post-submit) tx hash."
          });
        }
        return textResult({
          governed: true,
          ...
          evaluation_id: evalId,
          compass_payload: compass.data,
          note: "Approved by Sly, but the Compass response has no `transaction` or `typed_data` \u2014 wrapper has nothing to sign/broadcast automatically. If this was a step-up action, the one-shot grant remains active until you sign-typed-data / execute-intent manually."
        });
      }
