    ...
    try {
      ...
      if (spec) {
        ...
        if (spec.executable && decision.evaluation_id && compass.data?.transaction) {
          const d = compass.data;
          const safeAddress = d.credit_account_address ?? d.earn_account_address ?? d.tokenized_account_address;
          const exec = await executeIntent(ctx, {
            agent_id: args.agent_id,
            evaluation_id: decision.evaluation_id,
            chain: p.chain,
            unsigned_transaction: compass.data.transaction,
            ...safeAddress && /^0x[0-9a-fA-F]{40}$/.test(safeAddress) ? { safe_address: safeAddress } : {}
          });
