...
var tools = [
  ...
  // ── Governed: credit (gate + signable payload) ─────────────────────
  {
    ...
    description: "Borrow USDC against on-chain collateral via Compass credit, GATED by Sly. On approve returns the Permit2/EIP-712 payload to sign (credit uses a different signing flow than earn \u2014 not auto-broadcast). Deny \u2192 reasons.",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string" },
  ...
  // ── Governed: perps (gate + signable payload, Hyperliquid) ─────────
  {
    ...
    description: "Place a perpetuals market order (Hyperliquid global markets), GATED by Sly. On approve returns the Hyperliquid signable payload (perps use their own signature scheme \u2014 not CDP-broadcast). Size is gated as the at-risk amount. Deny \u2192 reasons.",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string" },
  ...
  // ── Governed: tokenized equities (gate + signable payload) ─────────
  {
    ...
    description: "Buy tokenized equity shares (Ondo ERC-20: TSLAon, AAPLon, \u2026) with USDC, GATED by Sly. On approve returns the EIP-712 order payload to sign. Deny \u2192 reasons.",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string" },
