#!/usr/bin/env bash
#
# scripts/onboard.sh — partner self-serve provisioning for petal-lane.
#
# Reads CORAL_API_KEY from .env.local, provisions a Coral Demo account +
# Coral Buyer Agent (Petal Lane) on your tenant, writes IDs + agent token back.
#
...
echo "→ Provisioning petal-lane against $API_URL …"
echo
...
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Coral Demo':
        print(a.get('id', '')); break
")
if [[ -z "$ACCOUNT_ID" ]]; then
  echo "  • Creating Coral Demo account…"
  ACCOUNT_RESP=$(post_json "/v1/accounts" '{
    "type": "business",
    "name": "Coral Demo",
    "metadata": { "onboarded_via": "petal-lane/scripts/onboard.sh" }
  }')
...
# ── 2. Coral Buyer Agent (Petal Lane) ──────────────────────────────────────────────
EXISTING=$(get_json "/v1/agents?search=Coral%20Buyer%20Agent&limit=5" | python3 -c "
import json, sys
d = json.load(sys.stdin)
arr = d.get('data', d) if isinstance(d.get('data', []), list) else d.get('data', {}).get('data', [])
for a in (arr if isinstance(arr, list) else []):
    if a.get('name') == 'Coral Buyer Agent (Petal Lane)':
        print(a.get('id', '')); break
")

AGENT_ID="$EXISTING"
AGENT_TOKEN="${CORAL_AGENT_TOKEN:-}"

if [[ -z "$AGENT_ID" ]]; then
  echo "  • Creating Coral Buyer Agent (Petal Lane)…"
  AGENT_RESP=$(post_json "/v1/agents" "{
    \"accountId\": \"$ACCOUNT_ID\",
    \"name\": \"Coral Buyer Agent (Petal Lane)\",
    \"description\": \"Buyer agent that initiates ACP checkouts against the Petal Lane storefront\",
...
d=json.load(sys.stdin); data=d.get('data',d); creds=data.get('credentials', d.get('credentials',{})); print(creds.get('token',''))")
  ...
  echo "  ⚠ Agent $AGENT_ID exists but CORAL_AGENT_TOKEN is missing — Sly only returns the token at creation."
...
lines = [
    "",
    "# === Auto-written by petal-lane/scripts/onboard.sh — do not edit by hand ===",
    "SLY_API_URL=$API_URL",
    f"CORAL_ACCOUNT_ID=$ACCOUNT_ID",
    f"CORAL_AGENT_ID=$AGENT_ID",
]
if "$AGENT_TOKEN":
    lines.append(f"CORAL_AGENT_TOKEN=$AGENT_TOKEN")
lines.append("# === End auto-written block ===")
block = "\n".join(lines) + "\n"
current = env_path.read_text() if env_path.exists() else ""
current = re.sub(
    r"\n?# === Auto-written by petal-lane/scripts/onboard\.sh.*?# === End auto-written block ===\n?",
...
echo
echo "✓ Onboarding complete. Run: pnpm install && pnpm dev   # → http://localhost:3210"
